...
**Детектор SCAN vs TEXT** (`is_scan_pdf`) считает **буквы** (`[A-Za-zА-Яа-яЁё]`, unicode), а не сырую длину текста, и сэмплирует страницы равномерно по всему документу. Причина: PDF с телом в CID-шрифтах без `ToUnicode` (типичный «Print To PDF») через extractable-слой отдаёт только цифры/реквизиты — по сырой длине это проходит порог 100 и маскирует неизвлекаемое тело, документ ошибочно уходит в `standard` вместо распознавания. По буквам такие страницы дают ≈0 → корректно ловится скан → VLM. Порог `SCAN_MIN_LETTERS_PER_PAGE` (дефолт 50, среднее по сэмплу). Откат к старому поведению: `SCAN_TEXT_METRIC=chars`. Решение видно в логе: `[scan-detect] metric=letters pages_sampled=k/N avg=X.X threshold=50 -> SCAN|TEXT`.
